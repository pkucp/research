...
%                         x(:,2,i,j)=optx1;
            ...
            fxbefore(3*rdir1con+rdir2con-3,gasel)=fx;
                ...
                c2=0;
                lamda=zeros(2*matrixd+2,20);
                theta=zeros(sum(carnum)*3,20);
                ...
                %                     disp(100);
                            ...
                            %         obj1=-r2*cap(BP(1),BP(2),d12_R,ga,N0)+pP*(BP(2)+BP(3))+pB*BP(1)-theta(1,k)*BP(1)-theta(2,k)*BP(2)-theta(3,k)*BP(3)-lamda(:,k,i)'*A1'*BP+rho/2*norm(A1'*BP+A2'*x2(:,k,i)-C)^2-M./(1+(exp(-cap(BP(1),BP(2),d12_R,ga,N0))));
                                    ...
                                    options = sdpsettings('verbose',0,'cachesolvers',1,'solver','fmincon','fmincon.Algorithm','trust-region-reflective','fmincon.MaxFunEvals',7000,'usex0',1,...
...
%                                     end
                                    syms B23_V
%                                     eqn = cap(B23_V,
                                    F221=[0<=BP21(1)<=B2_0,0<=BP21(2)<=P3_0,min(cap(BP21(1),BP21(2),...
                                        d12_R(relayi,relayj),ga(gasel),N0),cap(BP21(1),x(3,k,relayi,relayj),...
                                        d23_V(eachcari,eachcarj),ga(gasel),N0))>=2*c0];
                                    F222=[0<=BP22(1)<=B2_0,0<=BP22(2)<=P3_0,BP22(2)<=(d12_R(relayi,relayj)...
                                        /d23_V(eachcari,eachcarj))^ga(gasel)*x(3,k,relayi,relayj),cap(BP22(1),x(3,k,relayi,relayj),...
                                        d23_V(eachcari,eachcarj),ga(gasel),N0)<=2*c0];
                                    F223=[0<=BP23(1)<=B2_0,0<=BP23(2)<=P3_0,BP23(2)>=(d12_R(relayi,relayj)...
                                        /d23_V(eachcari,eachcarj))^ga(gasel)*x(3,k,relayi,relayj),cap(BP23(1),BP23(2),...
                                        d12_R(relayi,relayj),ga(gasel),N0)<=2*c0];
                                    F211=[0<=BP21(1)<=B2_0,0<=BP21(2)<=P3_0,min(cap(BP21(1),BP21(2),...
                                        d12_R(relayi,relayj),ga(gasel),N0),cap(BP21(1),x(3,k-1,relayi,relayj),...
                                        d23_V(eachcari,eachcarj),ga(gasel),N0))>=2*c0];
                                    F212=[0<=BP22(1)<=B2_0,0<=BP22(2)<=P3_0,BP22(2)<=(d12_R(relayi,relayj)...
                                        /d23_V(eachcari,eachcarj))^ga(gasel)*x(3,k-1,relayi,relayj),cap(BP22(1),...
                                        x(3,k-1,relayi,relayj),d23_V(eachcari,eachcarj),ga(gasel),N0)<=2*c0];
                                    F213=[0<=BP23(1)<=B2_0,0<=BP23(2)<=P3_0,BP23(2)>=(d12_R(relayi,relayj)...
                                        /d23_V(eachcari,eachcarj))^ga(gasel)*x(3,k-1,relayi,relayj),...
                                        cap(BP23(1),BP23(2),d12_R(relayi,relayj),ga(gasel),N0)<=2*c0];
...
%                                         sol221=optimize(F221,obj221,options);
                                        sol221=optimize(F221,obj221,opt);
...
%                                         sol222=optimize(F222,obj222,options);
                                        sol222=optimize(F222,obj222,opt);
...
%                                         sol223=optimize(F223,obj223,options);
                                        sol223=optimize(F223,obj223,opt);
...
%                                         sol211=optimize(F211,obj211,options);
                                        sol211=optimize(F211,obj211,opt);
...
%                                         sol212=optimize(F212,obj212,options);
                                        sol212=optimize(F212,obj212,opt);
...
%                                         sol213=optimize(F213,obj213,options);
                                        sol213=optimize(F213,obj213,opt);
                                        tonorm=-A2T(:,:,urvcnt)*x1(:,k,eachcari,eachcarj)+tonorm;
                    ...
                    for i=1:4
                        for j=1:carnum(i)
                            if d12_R(i,j)>0
                                ...
                                theta(3*rescrv,k+1)=theta(3*rescrv,k)*(1-alpha)+1*alpha;%-(pP-r(i,j)/log(2)*(d12_R(i,j)^(-ga(gasel))./N0+x(2,k,i,j)/x(1,k,i,j)))*alpha;
                                rescrv=rescrv+1;
                            end
                        end
                    end
